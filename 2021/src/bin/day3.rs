...
fn part1(input: &Input) -> isize {
    ...
    .unwrap();
    // TODO:  could use bitwise NOT/complement here
...
// 10111, 10101, 11100, 10000, and 11001. Then, consider the second bit of the 7 remaining numbers:
// there are more 0 bits (4) than 1 bits (3), so keep only the 4 numbers with a 0 in the second
// position: 10110, 10111, 10101, and 10000. In the third position, three of the four numbers have
// a 1, so keep those three: 10110, 10111, and 10101. In the fourth position, two of the three
// numbers have a 1, so keep those two: 10110 and 10111. In the fifth position, there are an equal
// number of 0 bits and 1 bits (one each). So, to find the oxygen generator rating, keep the number
// with a 1 in that position: 10111. As there is only one number left, stop; the oxygen generator
// rating is 10111, or 23 in decimal. Then, to determine the CO2 scrubber rating value from the
// same example above:
...
// 01111, 00111, 00010, and 01010. Then, consider the second bit of the 5 remaining numbers: there
// are fewer 1 bits (2) than 0 bits (3), so keep only the 2 numbers with a 1 in the second
// position: 01111 and 01010. In the third position, there are an equal number of 0 bits and 1 bits
// (one each). So, to find the CO2 scrubber rating, keep the number with a 0 in that position:
// 01010. As there is only one number left, stop; the CO2 scrubber rating is 01010, or 10 in
// decimal. Finally, to find the life support rating, multiply the oxygen generator rating (23) by
